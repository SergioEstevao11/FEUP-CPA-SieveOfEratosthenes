/*
 *  soespmd.cpp - spmd implementation of the sieve of erastothenes w/ parallel lookups
 *  CPA @ M.EIC, 2023
 *  Authors:
...
main(void)
{
    ...
    #pragma omp parallel num_threads(NUM_THREADS) private(sieve) reduction(+:computed)
    {
        ...
        constexpr auto increment = NUM_THREADS * segment_size;
        const auto begin = segment_size * thread_num;

        for (std::uint64_t low = begin; low <= N; low += increment) {
